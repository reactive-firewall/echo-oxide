...
pub fn handle_escape_codes(input: &str) -> String {
...
pub fn parse_handle_escape(input_args: &Args) -> bool {
	let do_escape_handling: bool;
	if input_args.disable_escape {
		do_escape_handling = false;
	} else {
		if input_args.enable_escape {
			do_escape_handling = true;
		} else {
			do_escape_handling = false;
		};
	};
...
pub fn echo_cmd(input_args: &Args) {
	// Determine escape code handling behavior
	let handle_escape: bool = parse_handle_escape(&input_args);
	if let Some(vec) = &input_args.inputs {
		for arg in vec {
